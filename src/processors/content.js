...
export default class Content extends Task {
  ...
  _processFile(dest, page, opts) {
    ...
    this.app.log.debug('rendering content for page', dest)
    var newPage = _.extend(page, {page, site: opts.config})
    if(dest.indexOf('_includes') > -1 || dest.indexOf('_layouts') > -1) return Promise.resolve();
    return this.app.get('renderer').render(ext, newPage.body, newPage)
      .then((content) => {
